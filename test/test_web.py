...
def _load_web_module():
    ...
    web_engine_path = (here.parent.parent / "src" / "protocol" / "web.py").resolve()
    if not web_engine_path.exists():
        raise FileNotFoundError(f"web.py not found at: {web_engine_path}")

    _install_import_stubs()

    module_name = "web_engine_under_test"
    ...
    sys.modules['buffer'] = _load_buffer_module()
...
def _load_buffer_module():
    here = Path(__file__).resolve()
    buffer_path = (here.parent.parent / "src" / "stream" / "buffer.py").resolve()
    if not buffer_path.exists():
        raise FileNotFoundError(f"Buffer.py not found at: {buffer_path}")

    _install_import_stubs()

    module_name = "buffer_dependency"
...
class TestWebStateMachine(unittest.TestCase):
    ...
    def setUp(self):
        self.engine = self.web_module.WebEngine()
